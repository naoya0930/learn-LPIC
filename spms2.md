...
- 開発者は，configure,Makefile.inというファイルを出力してパッケージに入れる
- 配布後，ダウンロードしたユーザはconfigureを実行する
  - Makefile.inを入力としたMakefileの作成を実行
  - 基本的に環境に対して1回の実行を前提としている
    - ビルド環境依存の値の確定
    - 機能、ライブラリ、ヘッダの有無
...
### installコマンド
- ファイルを指定された場所にコピーして権限を与える
- cpとは何が違う？
  - 権限が自由に設定できる
  - すでにファイルが存在する場合はそれを削除する


### 【7-16】 tarの挙動
- 更新が発生したアーカイブファイルの展開におけるtarの挙動
  - すでにあるファイルは削除しない。
  - 新しい名前のファイルは書き込みを実行
  - 同じ名前のファイルは上書きする
...
- lsdev
  - レガシー
  - IRQ,I/Oポート、DMAに関する情報を表示
  - ハードウェア自身の情報を収集
  - その割にIRQを見るのか。。。

### SMART
- self-monitorung, Analysos and Reporting Technology
- ディスクの状態を取得する
- `smartctl`コマンドによって参照できる
- 基本は/var/log/messageにログは出力
