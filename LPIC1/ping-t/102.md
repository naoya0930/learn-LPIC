...
- `menuentry`
  - 起動項目名
  - 記載例：`menuentry "{mame}" --class {} --hotkey {}{...}`
- `linux`...カーネルのパス、およびカーネルパラメータ
- `linuxefi`... efi環境ではこちらを使用
- `initrd`...初期RAMディスクの指定
- `initedefi`... efi環境ではこちらを使用
- `set root={}`...ルートパーティションの指定。
  - {ハードディスク,パーティション}で記載。
- `search`...UUIDやLABELを使用するためのオプション
- `insmod`...GRUBモジュールの読み込み
- `loopback`
- `chainloader`...他のbootloaderを読み込む
- `multiboot`
- `boot`...起動する。最後に記載することが多い
- `load_video`...ビデオカードの初期化
- `gfxmode`...解像度
- `terminal_input/terminal_output`
- `recordfail`...前回のブートの成否
- `if,else,fi`...条件付きのロジック
