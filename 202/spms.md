...
### NFSv3,NFSv4
- NFSv3
  - 111番でアクセスを受け、その後は動的ポートを利用する。RPCに強固に依存
  - ファイアウォール経由が難しい
  - `rpc.moutd`,`rpc.statd`,`rpc.locked`などが起動
  - ステートレス、クライアントの状況を記憶しない
  - UID,GIDでユーザ識別
  - クライアントごとに個別にマウント設定
  - ホストベース認証。暗号化しない
- NFSv4
  ...
  - 補助デーモンが不要
  - サーバがファイルロックを管理
  - ユーザ@ドメインでクライアントを管理する
  - ルートFSの配下にすべての共有情報を集約する
  - Kerberos認証可能
### かつて使われていたrpcサービス
- rpcind...ポートマッパー。こいつが111番で受付
- rpc.mount...マウント要求を処理
- rpc.nfsd...ファイル操作。2049番に固定
- rpc.lockd...突合を防ぐ
- rpc.statd...生存確認
- rpc.rqotad...ディスク容量の監視

### NFSv4になって登場したサービス
- rpc.imapd
  - NFSがユーザの共有をしないため、旧版のUIDでの通信が厳しい
  - NFSのサーバとクライアントが同じUIDになるとは限らない
  - 「ユーザ名@ドメイン」で管理する
- 具体的な管理
  - tanaka@sample.com
  - クライアントから「tanaka」が来たら、サーバ自身の/etc/passwdを検索
  - 一致するものをtanaka@sample.comをマッピング
- 10人ぐらいならこれで成立する
  - これ以上はLDAPやSSSを使用する。
