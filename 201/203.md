...
#### GRUB Legacy設定ファイル
- `/boot/gtub/menu.lst`
- grubのstage2が立ち上がってから表示されるメニュー
- カーネルやinitramdがどこにあるのかを示してあげることが目的
- ファイルの中身
- 共通設定
  - timeout...メニューを表示する時間
  - default...デフォルトで起動するエントリ番号
  - splashimage...メニュー表示の背景画像
  - hiddenmenu...メニューを表示せずに起動する
- 各メニューエントリの詳細
  - title...エントリ名
  ...
  - makeactive...ルートパーティションをアクティブ化
    - そのディスクのブートローダを使用する，ぐらいの意味
    - ※grubからlinuxを起動する場合は不要
  - chainloader...指定されたセクタの読み出しと実行
    - 他のブートローダを呼び出すための設定
    - ルートパーティションからのセクタを実行できる
    - +1は次のセクタの場所ぐらいの意味
      - root (hd0)でchainloader +1 を指定すると，/dev/sdaを呼ぶ
      - root (hd0,1)でchainloader +1を指定すると，/dev/dsa1を呼ぶ
    - ※ grubからlinuxを起動する場合は不要
#### rootパーティションの置き方
- ファイルシステムが無いため，ディスクを指定する
- `root (hdX,Y)`
  - X...ハードディスク番号(0スタート)
  - Y...パーティション番号(0スタート)
- これをデバイスマップと呼ぶ．
  - `/boot/grub/device.map`に書いてある．
  - このファイルはgrubにしか無い
  - `sudo grub-mkdevicemap`で作成することもできる
#### grubシェル
- GRUBブートローダのデバック，手動操作，ブート設定の確認
- 手動でブート設定を試す
- ブート設定ファイルのデバック
- ブートローダ再構築・救出
- ディスクパーティションの探索
- grubシェルコマンド
  - dhcp...DHCPを使ってネットワークインタフェースを初期化
  - ifconfig...ネットワークインタフェースを設定
  - parttype...パーティションタイプを変更する
  - password...メニューのパスワードを設定
  - serial...シリアルデバイスを設定
  - setkey...キーマップを設定
  - boot...OSを起動
  - cat...ファイル内容を表示
  - configfile...設定ファイルを読み込む
  - displayapm...APM情報を表示する
  - displaymem...メモリ設定を表示
  - find...ファイルを検索する
  - halt...システムを停止
  - help...ヘルプを表示する
  - initrd...初期RAMディスクをロードする
  - install...GRUB isntall
  - kernel...カーネルをロードする
  ...
  - setup...GRUBを自動的にインストール

### GRUB2
- `/boot/grub/grub.cfg`(BIOS)に設定ファイルがある
- `/boot/efi/EFI/<id>/grub.cfg`(UEFI)がある
- 設定ファイルは`grub-mkconfig`もしくは`grub2-mkconfig`で生成
  - この時参照するのは`/etc/default/grub`だったり，`/etc/grub.d/*`
#### `/etc/default/grub`のパラメータ
- GRUB_DEFAULT...デフォルトで起動するOSの番号/メニュータイトル
  - savedが指定されている場合は前回選択したものを起動
- GRUB_HIDDEN_TIMEOUT...メニューを表示せずに待機する秒数
- GRUB_HIDDEN_TIMEOUT_QUIET...trueならメニューを表示しない
- GRUB_TIMEOUT...デフォルトのOSを起動するまでの時間
- GRUB_CMDLINE_LINUX_DEFAULT...ブートオプション
- GRUB_CMDLINE_LINUX...カーネルに渡すブートオプション
### 起動オプションの設定
- grub legacyの場合は，`kernel`の後段に渡す
  - init=の後に書いた変数はinitに渡される．
- grub2の場合は，CMDLINEに書く
- 主要なパラメータ
  - init={PATH}...指定したパスのコマンドを初期プログラムとして指定
  - mem={nメガバイト}...物理メモリを指定する
  - nosmp..SMPマシンをシングルプロセッサとして動作させる
  - noht...Hyper Threading機能を無効にする
  - maxcpus={数}...利用するCPUの数を指定した数に制限する
  - root={デバイス}...ルートファイルシステムを指定する
  - ready-only,ro...ルートファイルシステムを読み取り専用でマウント
  - single,s...シングルユーザモードで起動
  - {数値}...指定した数値の欄レベルで起動
#### パラメータはどこに渡されたか？
- `cat /proc/cmdline`カーネルに渡された引数を確認
- `ps -o args -p 1`(PID=1)の引数確認
### システムの回復
- レスキューモードで起動すると，ルートファイルシステムのマウントポイントがずれる
  - `/mnt/sysimage/`直下にいつもの構造が作成される など
- rpm等でパッケージ管理しようとするとずれるのでコマンドで補正
  - `rpm -qf --root /mnt/sysimage`
- シェルが終了するまで，ルートディレクトリをずらすこともできる
  - `chroot /mnt/sysimage`
## その他のブートローダ
- 特定のファイルフォーマットにあるカーネルを引っ張り出す
...
### PXEブート
- interl社製のネットワークブート規格に対応したブート方式
  - ネットワークカードがPXE対応である必要がある
- DHCPやBOOTPを使用して，TFTPでブートプログラムをダウンロードして起動
- インストールメディアなしでカーネル起動
- DHCPを使うことがミソ
  - `next-server {TFTPサーバのIP}`
  - `filename "{起動イメージ/ブートローダのパス}"`で渡す
- TFTP...Trivial File Transfer Protocol
- ブートローダ
  - gtubの場合，pxlinux.0
  - grub2の場合，frub2.efi
- OSは一般にHTTPやNFSでインストールされやすい
